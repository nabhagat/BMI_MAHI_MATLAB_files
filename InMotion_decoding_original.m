...
if train_LDA_model == 1
    ...
    process_raw_eeg = 1;
    label_events = 1;       % process kinematics and label the events/triggers
    include_target_events = 1;
...
if process_raw_eeg == 1
    ...
    if common_avg_ref == 1
        EEG = pop_reref( EEG, [],'exclude',[1 2 17 22 28 32 41 46]);       % Exclude HEOG, VEOG channels
        %EEG = pop_reref( EEG, []);
        % EEG.setname='NB_CAR';
...
if label_events == 1
    ...
    kinematics_raw = importdata([folder_path Subject_name '_kinematics.txt']); % Raw data sampled at 200 Hz
    Fs_kin_raw = 200;
    % Set zero time
...
%% Generate Raster Plot
    ...
    set(gca,'YTickLabel',{'Fz','FC1','FC2','Cz','CP1','CP2','F1','F2','C1','C2','CPz','Pos X ','Pos Y','Tang Velocity'},'FontSize',8);
...
if extract_epochs == 1
    ...
    EEG = pop_epoch( EEG, {  '100'  }, [-2 1], 'newname', [Subject_name '_move_epochs'], 'epochinfo', 'yes');
    EEG = eeg_checkset( EEG );
    ...
    EEG = pop_epoch( EEG, {  '200'  }, [-1 0], 'newname', [Subject_name '_rest_epochs'], 'epochinfo', 'yes');
    EEG = eeg_checkset( EEG );
    EEG = pop_resample( EEG, 10);
...
for epoch_cnt = 1:no_epochs
    for channel_cnt = 1:no_channels
        %move_sum_channels(channel_cnt,:) = move_sum_channels(channel_cnt,:) + EEG.data(channel_cnt,:,epoch_cnt);
...
if plot_ERPs == 1
    ...
    xlabel('Time (sec.)')
    ylabel('Voltage (\muV)');
    legend('RP during Movement','Movement Onset','Orientation','Horizontal');
    ...
    for RPind = 1:5
        ...
        jbfill(move_erp_time,move_avg_channels(mychanns(RPind),:)+move_std_channels(mychanns(RPind),:),...
            move_avg_channels(mychanns(RPind),:)-move_std_channels(mychanns(RPind),:),'b','k',0,0.3);
        ...
        xlabel('Time (sec)');
        ylabel('\mu Volts');
        axis([move_erp_time(1) move_erp_time(end) -10 10]);
...
CVO = cvpartition(2*no_epochs,'kfold',5);
...
for cv_index = 1:CVO.NumTestSets
    ...
    [test_labels, Error, Posterior, LogP, OutputCoefficients] = ...
       classify(data_set(teIdx,:),data_set(trIdx,:),data_set_labels(trIdx,1), 'linear');

    CM = confusionmat(data_set_labels(teIdx,:),test_labels);
