...
Subject_name = 'FJ2';
%folder_path = ['F:\Nikunj_Data\InMotion_Experiment_Data\Subject_' Subject_name '_ses1_training\'];
folder_path = ['F:\Nikunj_Data\InMotion_Experiment_Data\Subject_' Subject_name ];
% Channels_nos = [38;5;39;9;32;10;48;14;49;19;53;20;13;15];
Channels_nos = [4 5 6 9 10 13 14 15 19 20 24 25 26 32 ...
...
% Set EEGLAB path to switch between different versions

% if laplacian_ref == 1
%     rmpath(genpath('F:\EEGlab'));
%     addpath(genpath('F:\BCILAB'));
%     %pathtool;
% else
%     rmpath(genpath('F:\BCILAB'));
%     addpath(genpath('F:\EEGlab'));
%     %pathtool;
% end
...
if process_raw_eeg == 1
    ...
    %EEG = pop_loadset( [Subject_name '_raw.set'], ['F:\Nikunj_Data\InMotion_Experiment_Data\Subject_' Subject_name]); % read in the dataset
    EEG = pop_loadset( [Subject_name '_raw.set'], folder_path); % read in the dataset
    ...
    hpfc = 0.02;     % Cutoff frequency = 0.1 Hz
    ...
    HPFred_eeg = zeros(size(EEG.data));
    for i = 1:EEG.nbchan
        %EEG.data(i,:) = detrend(EEG.data(i,:)); %Commented on 12-9-13
        %HPFred_eeg(i,:) = filtfilt(num_hpf,den_hpf,double(EEG.data(i,:)));
        HPFred_eeg(i,:) = filtfilt(eeg_hpf.sos.sosMatrix,eeg_hpf.sos.ScaleValues,double(EEG.data(i,:)));
    end
    EEG.data = HPFred_eeg;

    % ****STEP 3: Re-Reference data
    if common_avg_ref == 1
        EEG = pop_reref( EEG, [],'exclude',[1 2 17 22 41 46]);       % Exclude HEOG, VEOG channels
        %EEG = pop_reref( EEG, []);
        % EEG.setname='NB_CAR';
        EEG = eeg_checkset( EEG );
    elseif bipolar_ref == 1
        EEG_orig = double(EEG.data);
        EEG_bp   = EEG_orig;
        
        % Subtract Left electrodes from Right electrodes
%         EEG_bp(9,:) = EEG_orig(9,:) - EEG_orig(43,:);         % FC1
%         EEG_bp(48,:) = EEG_orig(48,:) - EEG_orig(13,:);       % C1
%         EEG_bp(14,:) = EEG_orig(14,:) - EEG_orig(48,:);       % Cz
%         EEG_bp(49,:) = EEG_orig(49,:) - EEG_orig(14,:);       % C2
%         EEG_bp(19,:) = EEG_orig(19,:) - EEG_orig(52,:);       % CP1
%         EEG_bp(53,:) = EEG_orig(53,:) - EEG_orig(19,:);       % CPz
%         EEG_bp(20,:) = EEG_orig(20,:) - EEG_orig(53,:);       % CP2
%         EEG_bp(38,:) = EEG_orig(38,:) - EEG_orig(4,:);        % F1
%         EEG_bp(5,:) = EEG_orig(5,:) - EEG_orig(38,:);         % Fz
%         EEG_bp(39,:) = EEG_orig(39,:) - EEG_orig(5,:);        % F2
%         EEG_bp(10,:) = EEG_orig(10,:) - EEG_orig(44,:);       % FC2

% Subtract in Longitudinal direction
        EEG_bp(9,:) = EEG_orig(9,:) - EEG_orig(38,:);         % FC1
        EEG_bp(48,:) = EEG_orig(48,:) - EEG_orig(9,:);        % C1
        EEG_bp(19,:) = EEG_orig(19,:) - EEG_orig(48,:);       % CP1
        
        EEG_bp(14,:) = EEG_orig(14,:) - EEG_orig(5,:);        % Cz
        EEG_bp(53,:) = EEG_orig(53,:) - EEG_orig(14,:);       % CPz
        
        EEG_bp(10,:) = EEG_orig(10,:) - EEG_orig(39,:);       % FC2
        EEG_bp(49,:) = EEG_orig(49,:) - EEG_orig(10,:);       % C2      
        EEG_bp(20,:) = EEG_orig(20,:) - EEG_orig(49,:);       % CP2
       
        EEG.data = EEG_bp;
        EEG.ref = 'bipolar';
        EEG.setname = 'bipolar-ref';
        EEG = eeg_checkset( EEG );   
        
    elseif laplacian_ref == 1
         ...
         % Create Large Laplacian Matrix
            ...
            for nn_row = 1:size(Neighbors,1) 
             ...
             EEG.data = L_LAP*(EEG.data);
             
%           EEG_orig = double(EEG.data);
%           EEG_glap = EEG_orig;
%           
          % Small laplacian
%           EEG_slap(38,:) = EEG_orig(38,:) - mean([EEG_orig(34,:);EEG_orig(4,:);EEG_orig(5,:);EEG_orig(9,:)],1);
%           EEG_slap(5,:) = EEG_orig(5,:) - mean([EEG_orig(28,:);EEG_orig(38,:);EEG_orig(39,:);EEG_orig(32,:)],1);
%           EEG_slap(39,:) = EEG_orig(39,:) - mean([EEG_orig(35,:);EEG_orig(5,:);EEG_orig(6,:);EEG_orig(10,:)],1);
%           EEG_slap(9,:) = EEG_orig(9,:) - mean([EEG_orig(38,:);EEG_orig(43,:);EEG_orig(48,:);EEG_orig(32,:)],1);
%           
%           EEG_slap(32,:) = EEG_orig(32,:) - mean([EEG_orig(5,:);EEG_orig(9,:);EEG_orig(10,:);EEG_orig(14,:)],1);
%           
%           EEG_slap(10,:) = EEG_orig(10,:) - mean([EEG_orig(39,:);EEG_orig(32,:);EEG_orig(49,:);EEG_orig(44,:)],1);
%           
%           EEG_slap(48,:) = EEG_orig(48,:) - mean([EEG_orig(9,:);EEG_orig(13,:);EEG_orig(14,:);EEG_orig(19,:)],1);
%           
%           EEG_slap(14,:) = EEG_orig(14,:) - mean([EEG_orig(32,:);EEG_orig(48,:);EEG_orig(53,:);EEG_orig(49,:)],1);
%           
%           EEG_slap(49,:) = EEG_orig(49,:) - mean([EEG_orig(14,:);EEG_orig(20,:);EEG_orig(15,:);EEG_orig(10,:)],1);
%           
%           EEG_slap(19,:) = EEG_orig(19,:) - mean([EEG_orig(48,:);EEG_orig(52,:);EEG_orig(57,:);EEG_orig(53,:)],1);
%           EEG_slap(21,:) = EEG_orig(21,:) - mean([EEG_orig(14,:);EEG_orig(19,:);EEG_orig(25,:);EEG_orig(20,:)],1);
%           EEG_slap(20,:) = EEG_orig(20,:) - mean([EEG_orig(49,:);EEG_orig(53,:);EEG_orig(58,:);EEG_orig(54,:)],1);
%           
%           EEG_slap(13,:) = EEG_orig(13,:) - mean([EEG_orig(43,:);EEG_orig(47,:);EEG_orig(52,:);EEG_orig(48,:)],1);
%           
%           EEG_slap(15,:) = EEG_orig(15,:) - mean([EEG_orig(44,:);EEG_orig(49,:);EEG_orig(54,:);EEG_orig(50,:)],1);

          % Generlaized Laplacian
%   EEG_glap(38,:) = EEG_orig(38,:) - mean([EEG_orig(1,:);EEG_orig(37,:);EEG_orig(48,:);EEG_orig(39,:);EEG_orig(34,:);EEG_orig(4,:);EEG_orig(5,:);EEG_orig(9,:);EEG_orig(28,:);EEG_orig(32,:);EEG_orig(43,:)],1);
%   EEG_glap(5,:) = EEG_orig(5,:) - mean([EEG_orig(4,:);EEG_orig(14,:);EEG_orig(6,:);EEG_orig(28,:);EEG_orig(38,:);EEG_orig(39,:);EEG_orig(32,:);EEG_orig(9,:);EEG_orig(10,:)],1);
%   EEG_glap(39,:) = EEG_orig(39,:) - mean([EEG_orig(2,:);EEG_orig(38,:);EEG_orig(49,:);EEG_orig(40,:);EEG_orig(35,:);EEG_orig(5,:);EEG_orig(6,:);EEG_orig(10,:);EEG_orig(28,:);EEG_orig(32,:);EEG_orig(44,:)],1);
% 
%   EEG_glap(9,:) = EEG_orig(9,:) - mean([EEG_orig(28,:);EEG_orig(8,:);EEG_orig(19,:);EEG_orig(10,:);EEG_orig(38,:);EEG_orig(43,:);EEG_orig(48,:);EEG_orig(32,:);EEG_orig(4,:);EEG_orig(13,:);EEG_orig(14,:);EEG_orig(5,:)],1);
%   
% 
%   EEG_glap(32,:) = EEG_orig(32,:) - mean([EEG_orig(28,:);EEG_orig(43,:);EEG_orig(53,:);EEG_orig(44,:);EEG_orig(5,:);EEG_orig(9,:);EEG_orig(10,:);EEG_orig(14,:);EEG_orig(38,:);EEG_orig(39,:);EEG_orig(48,:);EEG_orig(49,:)],1);
% 
%   EEG_glap(10,:) = EEG_orig(10,:) - mean([EEG_orig(28,:);EEG_orig(9,:);EEG_orig(11,:);EEG_orig(20,:);EEG_orig(5,:);EEG_orig(6,:);EEG_orig(14,:);EEG_orig(15,:)],1);
% 
%   EEG_glap(48,:) = EEG_orig(48,:) - mean([EEG_orig(38,:);EEG_orig(47,:);EEG_orig(57,:);EEG_orig(49,:);EEG_orig(9,:);EEG_orig(13,:);EEG_orig(14,:);EEG_orig(19,:);EEG_orig(32,:);EEG_orig(43,:);EEG_orig(52,:);EEG_orig(53,:);],1);
% 
%   EEG_glap(14,:) = EEG_orig(14,:) - mean([EEG_orig(5,:);EEG_orig(13,:);EEG_orig(25,:);EEG_orig(15,:);EEG_orig(32,:);EEG_orig(48,:);EEG_orig(53,:);EEG_orig(49,:);EEG_orig(9,:);EEG_orig(10,:);EEG_orig(19,:);EEG_orig(20,:)],1);
% 
%   EEG_glap(49,:) = EEG_orig(49,:) - mean([EEG_orig(39,:);EEG_orig(48,:);EEG_orig(58,:);EEG_orig(50,:);EEG_orig(14,:);EEG_orig(20,:);EEG_orig(15,:);EEG_orig(10,:);EEG_orig(32,:);EEG_orig(53,:);EEG_orig(54,:);EEG_orig(44,:)],1);
...
%           EEG_slap(15,:) = EEG_orig(15,:) - mean([EEG_orig(44,:);EEG_orig(49,:);EEG_orig(54,:);EEG_orig(50,:)],1);
          % Large laplacian
%           EEG_llap(38,:) = EEG_orig(38,:) - mean([EEG_orig(1,:);EEG_orig(37,:);EEG_orig(48,:);EEG_orig(39,:)],1);
%           EEG_llap(5,:) = EEG_orig(5,:) - mean([EEG_orig(4,:);EEG_orig(14,:);EEG_orig(6,:)],1);
%           EEG_llap(39,:) = EEG_orig(39,:) - mean([EEG_orig(2,:);EEG_orig(38,:);EEG_orig(49,:);EEG_orig(40,:)],1);
%           
%           EEG_llap(9,:) = EEG_orig(9,:) - mean([EEG_orig(28,:);EEG_orig(8,:);EEG_orig(19,:);EEG_orig(10,:)],1);
%           
%           EEG_llap(32,:) = EEG_orig(32,:) - mean([EEG_orig(28,:);EEG_orig(43,:);EEG_orig(53,:);EEG_orig(44,:)],1);
%           
%           EEG_llap(10,:) = EEG_orig(10,:) - mean([EEG_orig(28,:);EEG_orig(9,:);EEG_orig(11,:);EEG_orig(20,:)],1);
%           
%           EEG_llap(48,:) = EEG_orig(48,:) - mean([EEG_orig(38,:);EEG_orig(47,:);EEG_orig(57,:);EEG_orig(49,:)],1);
%           
%           EEG_llap(14,:) = EEG_orig(14,:) - mean([EEG_orig(5,:);EEG_orig(13,:);EEG_orig(25,:);EEG_orig(15,:)],1);
%           
%           EEG_llap(49,:) = EEG_orig(49,:) - mean([EEG_orig(39,:);EEG_orig(48,:);EEG_orig(58,:);EEG_orig(50,:)],1);
...
%         EEG = eeg_checkset( EEG ); 
    ...
    LPFred_eeg = zeros(size(EEG.data));
    for i = 1:EEG.nbchan
        %LPFred_eeg(i,:) = filtfilt(num_lpf,den_lpf,double(EEG.data(i,:)));
        LPFred_eeg(i,:) = filtfilt(eeg_lpf.sos.sosMatrix,eeg_lpf.sos.ScaleValues,double(EEG.data(i,:)));
    end
    EEG.data = LPFred_eeg;   

    % EEG = pop_iirfilt( EEG, 0, 1, [], 0, 0);
    % EEG = eeg_checkset( EEG );

    % **** STEP 5: Resample to 100 Hz 
    EEG = pop_resample( EEG, 100);
    EEG.setname=[Subject_name '_preprocessed'];
    EEG = eeg_checkset( EEG );
    % Save dataset
    EEG = pop_saveset( EEG, 'filename',[Subject_name '_preprocessed.set'],'filepath',folder_path);
    EEG = eeg_checkset( EEG );
...
%         [PSD_eeg(psd_len,:),PSD_f] = pwelch(EEG.data(Channels_nos(psd_len),:),[],[],NFFT,Fs_eeg);   
%         plot(PSD_f(1:50),10*log10(PSD_eeg(psd_len,1:50)),myColors(psd_len));
%         hold on;
%     end
%     xlabel('Frequency Hz');
%     ylabel('PSD dB');
...
% Save dataset
if standardize_data_flag == 1
    EEG.setname = [Subject_name '_standardized_event_labels'];
    EEG = pop_saveset( EEG, 'filename',[Subject_name '_standardized_event_labels.set'],'filepath',folder_path);
    EEG = eeg_checkset( EEG );
else
    EEG.setname = [Subject_name '_preprocessed_event_labels'];
    EEG = pop_saveset( EEG, 'filename',[Subject_name '_preprocessed_event_labels.set'],'filepath',folder_path);
    EEG = eeg_checkset( EEG );
end
...
if extract_epochs == 1
    ...
    % Extract move epochs
    if standardize_data_flag == 0
        EEG = pop_loadset( [Subject_name '_preprocessed_event_labels.set'], folder_path); % read in the dataset
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG); % copy it to ALLEEG
        eeglab redraw;
    else
        EEG = pop_loadset( [Subject_name '_standardized_event_labels.set'], folder_path); % read in the dataset
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG); % copy it to ALLEEG
        eeglab redraw;
    end
    EEG = pop_epoch( EEG, {  '100'  }, [-2 3.1], 'newname', [Subject_name '_move_epochs'], 'epochinfo', 'yes');
    EEG = eeg_checkset( EEG );
    EEG = pop_resample( EEG, 10);
    EEG.setname=[Subject_name '_move_epochs'];
    EEG = eeg_checkset( EEG );
    % Save dataset
    EEG = pop_saveset( EEG, 'filename',[Subject_name '_move_epochs.set'],'filepath',folder_path);
    EEG = eeg_checkset( EEG );
    % Update EEGLAB window
    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
    eeglab redraw;
    ...
    EEG = pop_epoch( EEG, {  '200'  }, [-2 3.1], 'newname', [Subject_name '_rest_epochs'], 'epochinfo', 'yes');
    EEG = eeg_checkset( EEG );
    EEG = pop_resample( EEG, 10);
    EEG.setname=[Subject_name '_rest_epochs'];
    EEG = eeg_checkset( EEG );
    % Save dataset
    EEG = pop_saveset( EEG, 'filename',[Subject_name '_rest_epochs.set'],'filepath',folder_path);
    EEG = eeg_checkset( EEG );
    Fs_eeg = EEG.srate;           % Sampling rate after downsampling of epochs
    % Update EEGLAB window
    [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
    eeglab redraw;
end
...
for epoch_cnt = 1:no_epochs
    for channel_cnt = 1:no_channels
        ...
        rest_mean_baseline(channel_cnt,epoch_cnt) = mean(rest_epochs(epoch_cnt,find(rest_erp_time == (-0.7)):find(rest_erp_time == (-0.0)),channel_cnt));     
...
for epoch_cnt = 1:no_epochs
    for channel_cnt = 1:no_channels
        ...
        move_epochs(epoch_cnt,:,channel_cnt) = move_epochs(epoch_cnt,:,channel_cnt) - rest_mean_baseline(channel_cnt,epoch_cnt);
        % %tmove_epochs(epoch_cnt,:,channel_cnt) = move_epochs(epoch_cnt,:,channel_cnt) - move_mean_baseline(channel_cnt,epoch_cnt);
    end
end
...
if plot_ERPs == 1
    ...
    hold on
    plot_ind4 = 1;
    for RPind = 1:length(Channels_nos(1:9))
%         if plot_ind4 == 5
%             plot_ind4 = plot_ind4 + 1;
%         end
        subplot(3,3,plot_ind4); hold on;
        ...
        line([0 0],[-6 6],'Color','k','LineWidth',2);  
        plot_ind4 = plot_ind4 + 1;
        grid on;
    %     xlabel('Time (sec.)')
    %     ylabel('Voltage (\muV)');
    ...
    xlabel('Time (sec.)')
    ylabel('Voltage (\muV)');
    legend('RP during Movement','Orientation','Horizontal');
    
    if common_avg_ref == 1
        title('Readiness Potential with CAR Montage','FontSize',14);
    elseif bipolar_ref == 1
        title('Readiness Potential with Bipolar Reference Montage','FontSize',14);
    elseif laplacian_ref == 1
        title('Readiness Potential with Laplacian Reference Montage','FontSize',14);
    end
