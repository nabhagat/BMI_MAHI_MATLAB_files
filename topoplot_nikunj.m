...
EMARKER2LINEWIDTH = 1;
EFSIZE = 10; %get(0,'DefaultAxesFontSize'); % use current default fontsize for electrode labels - nikunj
    ...
    for i = 1:2:length(varargin)
        ...
        switch Param
            ...
            case {'whitebk','whiteback','forprint'}
                whitebk = Value;
            case 'mrcp_chans'                                           % nikunj 1/5/15
                mrcp_chans = Value; 
            case 'classifier_chans'                                     % nikunj 1/5/15        
                classifier_chans = Value;
            otherwise
...
%
  ...
  if strcmp(ELECTRODES,'labelpoint') |  strcmp(ELECTRODES,'numpoint')
    ...
    set(tl, 'fontweight', 'bold','fontsize',12);
  end;
end % STYLE 'blank'
...
%
 plotax = gca;
 %axis square                    % nikunj 1/5/15                       % make plotax square
 axis off
 ...
 % axes(textax);                   
 %axis square                  % nikunj 1/5/15                         % make textax square
...
elseif strcmp(ELECTRODES,'labelpoint') 
    % change (x,y) values - nikunj 1/5/15
    x = x + [0.02 0.02  0   0   0 ...
                -0.02 -0.02 0.02 0.02 0.02...
                0 0 -0.02 -0.02 -0.02];
    y = y + [-0.02 0.02 -0.04 0 0.04 ...
                   -0.02 +0.02 0 -0.04 0.04 ...
                   -0.02 0.02 -0.04 0 0.04];
  if isempty(EMARKER2CHANS)
    hp2 = plot3(y,x,ones(size(x))*ELECTRODE_HEIGHT,...
        EMARKER,'Color',ECOLOR,'markersize',EMARKERSIZE,'linewidth',EMARKERLINEWIDTH);
    for i = 1:length(mrcp_chans) % nikunj - 1/5/15
        mrcp_ch_ind = find(plotchans == mrcp_chans(i));
        hp21 = plot3(y(mrcp_ch_ind),x(mrcp_ch_ind),ELECTRODE_HEIGHT,...
            EMARKER,'Color',[0 0 1],'markersize',EMARKERSIZE+6,'linewidth',EMARKERLINEWIDTH);
    end
    for j = 1:length(classifier_chans) % nikunj - 1/5/15
        classifier_ch_ind = find(plotchans == classifier_chans(j));
        %hp22 = plot3(y(classifier_ch_ind),x(classifier_ch_ind),ELECTRODE_HEIGHT,...
         %   EMARKER,'Color',[1 0 0],'markersize',EMARKERSIZE+6,'linewidth',EMARKERLINEWIDTH);
         %if j == 1 || j == 2 %- BNBO_ses2_cond1
         if j == 2 % BNBO_ses2_cond3 
             line([x(classifier_ch_ind)-0.05 x(classifier_ch_ind)+0.05],[y(classifier_ch_ind)-0.05 y(classifier_ch_ind)-0.05],[ELECTRODE_HEIGHT ELECTRODE_HEIGHT],...
             'Color',[1 0 0],'linewidth',2);
         %elseif j == 3 %- BNBO_ses2_cond1
             %line(-1.*[x(classifier_ch_ind) - 0.05 x(classifier_ch_ind)+0.05],[-1.*y(classifier_ch_ind)-0.05 -1.*y(classifier_ch_ind)-0.05],[ELECTRODE_HEIGHT ELECTRODE_HEIGHT],...
             %'Color',[1 0 0],'linewidth',2);
         %elseif j == 4  %- BNBO_ses2_cond1
         elseif (j == 1) || (j==3) || (j ==4) % BNBO_ses2_cond3
          line([y(classifier_ch_ind) - 0.05 y(classifier_ch_ind)+0.05],[x(classifier_ch_ind)-0.05 x(classifier_ch_ind)-0.05],[ELECTRODE_HEIGHT ELECTRODE_HEIGHT],...
             'Color',[1 0 0],'linewidth',2);
         end
    end
  else
  ...
  label_offset = [-0.025 -0.025 -0.02 -0.02 -0.02 ...
                             -0.025 -0.025 -0.025 -0.025 -0.025 ...
                             -0.02 -0.02 -0.025 -0.025 -0.025];
  for i = 1:size(labels,1)
    hh(i) = text(double(y(i) + label_offset(i)),double(x(i)),...
