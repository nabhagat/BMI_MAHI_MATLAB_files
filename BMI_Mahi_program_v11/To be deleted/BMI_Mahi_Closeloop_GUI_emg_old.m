...
function pushbutton_start_closeloop_Callback(hObject, eventdata, handles)
    ...
    classifier_channels = CloopClassifier.classchannels(1:4);
...
try       
                    ...
                    raster_data = [All_Feature_Vec(1:3,1:length(pred_Ovr_Spatial_Avg)); [zeros(1,50) All_Feature_Vec(4,51:length(pred_Ovr_Spatial_Avg))]; pred_Ovr_Spatial_Avg;...
                                            [zeros(4,50) pred_Proc_EMG(:,51:length(pred_Ovr_Spatial_Avg))]]'; % Needs to be corrected
                    ...
                    axis(handles.axes_raster_plot,[pred_start_stop_times(1) pred_start_stop_times(2) 1 60]);
                    %ylim([0 40]);
...
function EEGTimer_Callback(timer_object,eventdata,hObject)
    ...
    global emg_abs_diff_pkt emg_mvc_pkt
    global prev_emg_abs_diff_pkt prev_emg_mvc_pkt
        ...
        %while ~finish
            try
                ...
                %% Initialize, Setup information like EEG properties
                            ...
                            emg_bp_filt_pkt = zeros(length(emg_classifier_channels),pkt_size);      % Use only EMG channels
                            prev_emg_bp_filt_pkt = zeros(length(emg_classifier_channels),pkt_size);
                            spatial_filt_pkt = zeros(total_chnns,pkt_size);
                            prev_spatial_filt_pkt = zeros(total_chnns,pkt_size);
                            emg_abs_diff_pkt = zeros(2,pkt_size);
                            emg_mvc_pkt = zeros(2,pkt_size);
                            prev_emg_abs_diff_pkt = zeros(2,pkt_size);
                            prev_emg_mvc_pkt = zeros(2,pkt_size);
                ...
                %% Process EEG, create feature vector, SVM classify, 32Bit Data block
                            ...
                            if firstblock == true
                                ...
                                if start_prediction == true                                                                               % change11
...
%--------------------------------------Add EMG filters here. 
                               % EMG Bandpass Filter
                               new_emg_pkt = EEGData(emg_classifier_channels,:);
                               emg_pkt_size = size(new_emg_pkt,2);
                               for num_chns = 1:length(emg_classifier_channels)
                                    emg_bp_filt_pkt(num_chns,:) = pkt_bp_filter(new_emg_pkt(emg_classifier_channels(num_chns),:),...
                                       prev_emg_pkt(emg_classifier_channels(num_chns),(emg_pkt_size - filt_order)+1:emg_pkt_size),...
                                       prev_emg_bp_filt_pkt(num_chns,(emg_pkt_size - filt_order)+1:emg_pkt_size),...
                                       orig_Fs);
                               end

                               emg_abs_diff_pkt = [abs(emg_bp_filt_pkt(1,:) - emg_bp_filt_pkt(3,:)); 
                                                           abs(emg_bp_filt_pkt(2,:) - emg_bp_filt_pkt(4,:))];
...
%                                % EMG Bandpass Filter
%                                for num_chns = 1:length(emg_classifier_channels)
%                                     emg_bp_filt_pkt(num_chns,:) = pkt_bp_filter(new_pkt(emg_classifier_channels(num_chns),:),...
%                                        prev_pkt(emg_classifier_channels(num_chns),(pkt_size - filt_order)+1:pkt_size),...
%                                        prev_emg_bp_filt_pkt(num_chns,(pkt_size - filt_order)+1:pkt_size),...
%                                        orig_Fs);
%                                end
% 
%                                emg_abs_diff_pkt = [abs(emg_bp_filt_pkt(1,:) - emg_bp_filt_pkt(3,:)); 
...
%                                for num_chns = 1:2
%                                    emg_mvc_pkt(num_chns,:) = pkt_lp_filter(emg_abs_diff_pkt(num_chns,:),...
%                                        prev_emg_abs_diff_pkt(num_chns,(pkt_size - filt_order)+1:pkt_size),...
%                                        prev_emg_mvc_pkt(num_chns,(pkt_size - filt_order)+1:pkt_size),...
%                                        orig_Fs);
%                                end
%                            Filtered_EMG_Downsamp = downsample(emg_mvc_pkt',downsamp_factor)';                                
