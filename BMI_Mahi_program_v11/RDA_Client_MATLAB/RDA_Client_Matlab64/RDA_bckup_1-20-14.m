...
function [unprocessed_eeg, processed_eeg,move_command,rest_command,marker_block,likert_score] = RDA(subj_name,filepath,serial_obj)
            ...
            while ~isempty(tryheader)
                ...
                switch hdr.type
                    case 1       % Start, Setup information like EEG properties
                        ...
                        pkt_size =  int32((window_size*100)/2);
                         filt_order = 4;
...
%                                     pt = length(move_command);
%                                     move_command = [move_command(1:(pt-valid_move_cnt+1)); ones(valid_move_cnt,1)];
 %                                    rest_command = [rest_command(1:(pt-valid_move_cnt+1)); zeros(valid_move_cnt,1)];                                     
