% Optimal channel selection using mutual information criteria
smart_move_ch_avg = [];
move_ch_avg_time = [];
smart_rest_ch_avg = [];
rest_ch_avg_time = [];
orig_channel_pool = classchannels; 
remaining_channel_pool = orig_channel_pool;
optimized_channels = [];
prev_MI = 0;

while length(optimized_channels) <= length(orig_channel_pool)
    Channel_comb_matrix = [];
    if ~isempty(optimized_channels)
        remaining_channel_pool(remaining_channel_pool == optimized_channels(end)) = [];
        total_no_combs = length(remaining_channel_pool);
        Channel_comb_matrix = [repmat(optimized_channels,total_no_combs,1) remaining_channel_pool'];
    else
        total_no_combs = length(remaining_channel_pool);
        Channel_comb_matrix = remaining_channel_pool';
    end
...
%         else
%             % select 4 channels with highest MI
%             topNchannels = 4;
%             [sorted_MI,sorted_MI_ind] = sort(MI_comb,'descend');
%             optimized_channels = Channel_comb_matrix(sorted_MI_ind(1:topNchannels))';
%             remaining_channel_pool = Channel_comb_matrix(sorted_MI_ind(topNchannels+1:end))';
%         end
    else
        % stop!!
        break
