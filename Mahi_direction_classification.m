...
folder_path = ['\\bmi-nas-01\contreras-UH\NRI_Project_Nikunj\Experiment_Data\All_Subjects\Subject_' Subject_name '\' Subject_name '_Session' num2str(Sess_num) '\']; % change2
closeloop_folder_path = ['\\bmi-nas-01\contreras-UH\NRI_Project_Nikunj\Experiment_Data\All_Subjects\Subject_' Subject_name '\' Subject_name '_Session' num2str(closeloop_Sess_num) '\']; % change3
save_folder_path = ['F:\Nikunj_Data\Direction_data'];  
...
%1. Spatial Filter Type
    ...
    common_avg_ref = 1; car_chnns_eliminate = [17 22 41 46]; % car_chnns_eliminate = [];  
...
bpfc_banks = [0.1 4; 
                ...
                14 25;
                26 40];
use_noncausal_filter = 1; % 1 - yes, use zero-phase filtfilt(); 0 - No, use filter()            %change6
...
move_epoch_dur = [-2.5 1]; % [-4 12.1];
rest_epoch_dur = [-2.5 1];
...
if process_raw_eeg == 1  
    ...
    %          'conv','off');
        ...
        % ****STEP 2: Band Pass Filter data, different filter banks, 4th order Butterworth
    %     if use_fir_filter ==  1
    %         num_hpf = fir_hp_filter;
    %         den_hpf = 1;
    %     else
        [num_bpf,den_bpf] = butter(2,(bpfc_banks(fb,:)/(raw_eeg_Fs/2)),'bandpass');
    %    end
        ...
        fvtool(num_bpf,den_bpf,'Analysis','freq','Fs',raw_eeg_Fs);  % Essential to visualize frequency response properly.
        ...
        for i = 1:eeg_nbchns
            ...
            if use_noncausal_filter == 1
                ...
                BPFred_eeg(i,:) = filtfilt(num_bpf,den_bpf,double(raw_eeg(i,:))); % filtering with zero-phase delay
            else
                BPFred_eeg(i,:) = filter(num_bpf,den_bpf,double(raw_eeg(i,:)));             % filtering with phase delay 
            end
        end
        %EEG.data = HPFred_eeg;
    ...
    %      end
        ...
        % Save dataset
        if ~isempty(Cond_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_eeg_preprocessed' num2str(fb) '.set'],'filepath',save_folder_path);    
        elseif ~isempty(Block_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_preprocessed.set'],'filepath',save_folder_path);    
        else
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_preprocessed.set'],'filepath',save_folder_path);
        end
...
if extract_epochs == 1
    for fb = 1:size(bpfc_banks,1)
        %% Extract move epochs
        if ~isempty(Cond_num)
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_eeg_preprocessed' num2str(fb) '.set'], save_folder_path);
        elseif ~isempty(Block_num)
            %EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_preprocessed_event_labels.set'], save_folder_path);
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_preprocessed.set'], save_folder_path);
        else
            %EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_preprocessed_event_labels.set'], save_folder_path);
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_preprocessed.set'], save_folder_path);
        end
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG); % copy it to ALLEEG
        eeglab redraw;        
    ...
    %     end
        ...
        EEG.setname=[Subject_name '_ses' num2str(Sess_num) '_move_epochs' num2str(fb)];
        EEG = eeg_checkset( EEG );

        % Save dataset
        if ~isempty(Cond_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_move_epochs' num2str(fb) '.set'],'filepath',save_folder_path);    
        elseif ~isempty(Block_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_move_epochs.set'],'filepath',save_folder_path);    
        else
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_move_epochs.set'],'filepath',save_folder_path);
        end
        % Update EEGLAB window
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
        eeglab redraw;
    ...
    %remove_corrupted_epochs = [81];
        ...
        EEG.setname=[Subject_name '_ses' num2str(Sess_num) '_rest_epochs' num2str(fb)];
        EEG = eeg_checkset( EEG );

        % Save dataset
        if ~isempty(Cond_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_rest_epochs' num2str(fb) '.set'],'filepath',save_folder_path);
        elseif ~isempty(Block_num)
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_rest_epochs.set'],'filepath',save_folder_path);    
        else
        EEG = pop_saveset( EEG, 'filename',[Subject_name '_ses' num2str(Sess_num) '_rest_epochs.set'],'filepath',save_folder_path);
        end
        % Update EEGLAB window
        [ALLEEG EEG CURRENTSET] = eeg_store(ALLEEG, EEG);
        eeglab redraw;
...
if manipulate_epochs == 1
    for fb = 1:length(bpfc_banks)
        if ~isempty(Cond_num)
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_rest_epochs' num2str(fb) '.set'], save_folder_path);
        elseif ~isempty(Block_num)
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_rest_epochs.set'], save_folder_path);
        else
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_rest_epochs.set'], save_folder_path);
        end
        Fs_eeg = EEG.srate;
    ...
    %     end
        ...
        if ~isempty(Cond_num)
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_move_epochs' num2str(fb) '.set'], save_folder_path);
        elseif ~isempty(Block_num)
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_block' num2str(Block_num) '_move_epochs.set'], save_folder_path);
        else
            EEG = pop_loadset( [Subject_name '_ses' num2str(Sess_num) '_move_epochs.set'], save_folder_path);
        end
    ...
    All_move_epochs = [All_move_epochs; move_epochs];
    All_rest_epochs = [All_rest_epochs; rest_epochs];
    end
end
...
%    Average.rest_epochs = rest_epochs;
   All_epochs.Fs_eeg = Fs_eeg;
   All_epochs.All_move_epochs = All_move_epochs;
   All_epochs.All_rest_epochs = All_rest_epochs;
   %All_epochs.apply_baseline_correction = apply_baseline_correction;
...
%    Average.move_mean_baseline = move_mean_baseline;   
   All_epochs.move_erp_time = move_erp_time;     
...
%    Average.rest_mean_baseline = rest_mean_baseline;   
   All_epochs.rest_erp_time = rest_erp_time;
...
%    end
   ...
   filename1 = [save_folder_path '\' Subject_name '_ses' num2str(Sess_num) '_cond' num2str(Cond_num) '_block' num2str(Block_num) '_all_epochs.mat'];
   save(filename1,'All_move_epochs','All_rest_epochs');   
end
